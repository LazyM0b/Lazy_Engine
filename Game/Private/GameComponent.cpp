...
void GameComponent::Initialize(Microsoft::WRL::ComPtr<ID3D11Device> device, MeshTypes type, MaterialTypes matType, std::vector<Vector4> colors, UINT detailsLVL) {
	this->type = type; 
	float phi = (1.0f + sqrt(5.0f)) * 0.5f; // golden ratio
	float coord1 = 1.0f;
	float coord2 = 1.0f / phi;
	...
	case Plastic:
		...
		properties->material.diffuse = Vector4(0.01f, 0.01f, 0.01f, 1.0f);
		properties->material.specular = Vector4(0.5f, 0.5f, 0.5f, 30.0f);
		break;
	case Metal:
		properties->material.ambient = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
...
void GameComponent::Initialize(Microsoft::WRL::ComPtr<ID3D11Device> device, MaterialTypes matType, const std::string& modelPath, const wchar_t* texturePath)
{
	this->type = Mesh;
	...
	case Plastic:
		properties->material.ambient = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
		properties->material.diffuse = Vector4(0.01f, 0.01f, 0.01f, 1.0f);
		properties->material.specular = Vector4(0.5f, 0.5f, 0.5f, 1.0f);
		break;
	case Metal:
		properties->material.ambient = Vector4(1.0f, 1.0f, 1.0f, 1.0f);
		properties->material.diffuse = Vector4(0.2775f, 0.2775f, 0.2775f, 1.0f);
		properties->material.specular = Vector4(0.773911f, 0.773911f, 0.773911f, 1.0f);
		break;
	default:
		break;
	}
...
void GameComponent::SphereSubdivide(std::vector<Vertex>& points, std::vector<int>& indeces)
{
	...
	for (UINT i = 0; i < indeces.size() / 3; ++i) {
